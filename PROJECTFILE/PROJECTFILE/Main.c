...
int AdminMenu(Position loggedIn);
int AdminAdjustPrice();
...
int AdminAdjustPrice() {
	...
	do {
		...
		if (choice <= NUMBER_OF_ROUTES) {
			...
			Insert(head, selected->HourFilename, &price);
			printf("The base price of a ticket on this line is: %.2f euros \n", price);
			...
			RewriteFile(head, selected->HourFilename, newPrice);
